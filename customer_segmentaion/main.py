...
def load_models():
    global model, disease_encoder, symptom_encoder_data, symptom_to_int, unique_symptoms
    try:
        if os.path.exists('xgboost_disease_model.joblib') and os.path.exists('disease_label_encoder.joblib') and os.path.exists('symptom_encoder.joblib'):
            ...
            print("[WARNING] Model assets not found. Run disease_prediction.py first to train the model.")
...
def health():
    ...
    return {
        "status": status,
        "model": "XGBoost Disease Classifier",
        "assets_exist": {
            "model": os.path.exists('xgboost_disease_model.joblib'),
...
def predict_disease(req: SymptomRequest):
    ...
    try:
        # Prepare input of 17 symptoms (padded with 'None')
        padded_symptoms = req.symptoms[:17]
